...
class VGG():
    ...
    def __init__(self, config, inputs):
        ...
        if config.saliency:  # rgbs or rgb
            ...
            self.s = tf.placeholder(tf.float32, shape=[None, self.image_size, self.image_size, 1], name='s')
    ...
    def build_model(self):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected], activation_fn=None,
            ...
            with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
                ...
                # vgg - conv
                for i in range(len(strides)):
                    net = slim.conv2d(net, filters[i], 3, stride=strides[i], padding='SAME',
                                      scope='sconv{}'.format(i))
                    net = slim.batch_norm(net, scope='sbn{}'.format(i))
                    print(net.shape)

                # vgg - fc, (=1x1 conv)

                net = slim.conv2d(net, hiddens[0], gp, stride=1, padding='VALID', scope='sfc{}'.format(i + 1))
                net = slim.batch_norm(net, scope='sbn{}'.format(i + 1))
                net = slim.dropout(net, keep_prob=0.25, is_training=self.is_training,
                                   scope='sdropout{}'.format(i + 1))
                print(net.shape)
                net_s = net

                print('########## concat')
                net = tf.concat([slim.flatten(net_x), slim.flatten(net_s)], axis=1)
                print(net.shape)

                net = slim.fully_connected(net, hiddens[1], scope='cfc{}'.format(i + 2))
                net = slim.batch_norm(net, scope='cbn{}'.format(i + 2))
                net = slim.dropout(net, keep_prob=0.5, is_training=self.is_training,
                                   scope='cdropout{}'.format(i + 2))
                print(net.shape)
                net = slim.fully_connected(net, self.class_num, scope='cfc{}'.format(i + 3))
                print(net.shape)
